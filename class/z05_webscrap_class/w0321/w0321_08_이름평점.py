...
year_rate_avg = [0]*3 # 년도별 평점 합계 평균
for y_i,y in enumerate(range(2021,2024)):
    ...
    print(f'{y}년도 평점합계평균 :',('{:.2f}'.format(avg)))
    year_rate_sum[y_i] = sum
    year_rate_avg[y_i] = float('{:.2f}'.format(avg))
        
    
print('영화 개수 :',len(m_list))
print('년도별 평점합계 :',year_rate_sum)
print('년도별 평점합계평균 :',year_rate_avg)